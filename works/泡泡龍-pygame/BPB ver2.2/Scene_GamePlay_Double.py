...
class Scene_GamePlay_Double:
    ...
    Img_BG = pygame.image.load("Graphics/bg0003.png").convert()
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@PC新增部分@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def PC_shoot_ball(self,x,y):#電腦發射(已完成)
        ...
        self.CURRENT_BALL2.append(randint(0,len(self.BALL_COLOR_BG)-1))
    ...
    def PC_search_space(self):#搜尋最近的空格擺放發射的球(已完成)
        ...
        if (self.BALL_DAMAGE > 0):
            ...
            self.ComboCount_RIVAL = 0
        #DAMAGE處理
        if ((self.DAMAGE_RIVAL > 1 and self.ComboCount_RIVAL == 0) or self.find_lowest_ball2() < 6):
            while (self.DAMAGE_RIVAL > 1 or self.find_lowest_ball2() < 6):
    ...
    def search_space(self):#搜尋最近的空格擺放發射的球(效能提升)
        ...
        if ((self.DAMAGE_LOCAL > 1 and self.ComboCount_LOCAL == 0) or self.find_lowest_ball() < 6):
            while (self.DAMAGE_LOCAL > 1 or self.find_lowest_ball() < 6):
